...
export default function App() {
  ...
  return <>
    ...
    {view === 'privacy' ? <PrivacyPolicy onBack={()=>navigate(auth.user?'lobby':'home')}/> : auth.user && auth.profile ? <section id="chatApp"><div className="layout-grid"><ProfileSidebar profile={auth.profile} friendRequests={friends.incoming.length} openReports={safety.reports.filter(r=>r.status==='open').length} onRules={() => setModal(INFO.rules)} onNavigate={navigate}/><div className="main-view">
      {['lobby','random','nostalgia'].includes(view) && <ChatRoom room={room} currentUsername={auth.profile.username} mentionUsers={[auth.profile,...people.people]} messages={chat.messages} status={`${chat.status} ${settings.status}`.trim()} onSend={chat.send} onSlowMode={settings.updateSlowMode} showChatProfileBorders={settings.showChatProfileBorders} pingsEnabled={chat.pingsEnabled} onEnablePings={chat.enablePings} lastSentAt={chat.lastSentAt} serverRemaining={chat.serverRemaining} slowModeSeconds={settings.slowModeSeconds} showChatProfileBorders={settings.showChatProfileBorders} isModerator={moderator} onReport={(message)=>setReportTarget({label:'message',userId:message.user_id,messageId:message.id})} onViewProfile={(person) => { setSelectedPerson(person); setView('profile'); }}/>} 
      ...
      {view === 'moderation' && moderator && <ModeratorPanel reports={safety.reports} status={`${safety.status} ${settings.status}`.trim()} slowModeSeconds={settings.slowModeSeconds} showChatProfileBorders={settings.showChatProfileBorders} onSlowMode={settings.updateSlowMode} onChatProfileBorders={settings.updateChatProfileBorders} onReview={safety.review} onDeleteMessage={safety.deleteMessage}/>} 

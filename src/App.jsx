...
export default function App() {
  ...
  const activeEvent=community.events.find(event=>event.enabled&&new Date(event.starts_at)<=new Date()&&new Date(event.ends_at)>new Date());
  ...
  useEffect(()=>{document.documentElement.dataset.cursor=auth.profile?.cursor_style||'classic_green';document.body.dataset.event=activeEvent?.theme||''},[auth.profile?.cursor_style,activeEvent?.theme]);
  ...
  return <>
    ...
    <main className="page-shell">{activeEvent&&<aside className="site-event-banner"><b>★ {activeEvent.title} ★</b><span>{activeEvent.message}</span></aside>}<section className="hero"><div><h1>Welcome to <span>symbiosis</span></h1><p>Talk to your people. Share weird stuff. Stay online.</p></div><div className="hero-badge">★ NEW &amp; IMPROVED ★</div></section>
    {view === 'privacy' ? <PrivacyPolicy onBack={()=>navigate(auth.user?'lobby':'home')}/> : auth.user && auth.profile ? activeSiteBan?<section className="panel banned-screen"><div className="panel-title">ACCESS TERMINATED</div><div className="panel-body"><h1>You are banned from Symbiosis.</h1><p>{activeSiteBan.reason||'No reason supplied.'}</p>{activeSiteBan.expires_at&&<p>Ends: {new Date(activeSiteBan.expires_at).toLocaleString()}</p>}</div></section>:<section id="chatApp"><div className="layout-grid"><ProfileSidebar profile={auth.profile} friendRequests={friends.incoming.length} openReports={safety.reports.filter(r=>r.status==='open').length} favoriteRooms={community.rooms.filter(room=>community.favorites.includes(room.slug))} onRules={() => setModal(INFO.rules)} onNavigate={navigate} onOpenRoom={jumpToRoom}/><div className="main-view">
      {roomRecord && <ChatRoom room={room} roomBanner={roomRecord.banner_url} pinnedNotice={roomRecord.pinned_notice} userId={auth.user.id} currentUsername={auth.profile.username} mentionUsers={[auth.profile,...people.people]} availableRooms={community.rooms} chatFramesEnabled={settings.chatFramesEnabled} targetMessageId={targetMessageId} messages={chat.messages} status={`${chat.status} ${settings.status}`.trim()} onSend={chat.send} onDelete={moderator?safety.deleteMessage:chat.deleteMessage} canDeleteAny={moderator} onSlowMode={seconds=>community.updateRoom(room,{slow_mode_seconds:seconds})} onLoadOlder={chat.loadOlder} hasOlder={chat.hasOlder} loadingOlder={chat.loadingOlder} pingsEnabled={chat.pingsEnabled} onEnablePings={chat.enablePings} lastSentAt={chat.lastSentAt} serverRemaining={chat.serverRemaining} slowModeSeconds={roomRecord.slow_mode_seconds} mediaMode={roomRecord.media_mode} isModerator={moderator} onJumpMessage={jumpToMessage} onJumpRoom={jumpToRoom} onJumpBulletin={jumpToBulletin} onReport={(message)=>setReportTarget({label:'message',userId:message.user_id,messageId:message.id})} onViewProfile={(person) => { setSelectedPerson(person); setView('profile'); }}/>}
      {view==='rooms'&&<RoomsPage rooms={community.rooms} favorites={community.favorites} currentUserId={auth.user.id} isStaff={moderator} onOpen={jumpToRoom} onCreate={community.createRoom} onUpdate={community.updateRoom} onFavorite={community.toggleFavorite} status={community.status}/>}
      ...
      {view === 'dms' && <DirectMessages
        user={auth.user}
        isOnline={presence.isOnline}
        chatFramesEnabled={settings.chatFramesEnabled}
        people={friends.friends.filter(person=>!safety.isBlocked(person.id))}
        requests={friends.incoming.filter(request=>!safety.isBlocked(request.person.id))}
        friendsApi={friends}
        initialPerson={selectedPerson}
        onViewProfile={(person) => { setSelectedPerson(person); setView('profile'); }}
      />}
      ...
      {view === 'moderation' && moderator && <ModeratorPanel reports={safety.reports} deletionLogs={community.logs} actions={community.actions} events={community.events} people={people.people} rooms={community.rooms} canManageTags={tags.includes('OWNER')} status={`${safety.status} ${community.status}`.trim()} chatFramesEnabled={settings.chatFramesEnabled} onChatFrames={settings.updateChatFrames} onReview={safety.review} onDeleteMessage={safety.deleteMessage} onModerate={community.moderate} onRevoke={community.revokeAction} onUpdateRoom={community.updateRoom} onUpdateTags={community.updateTags} onCreateEvent={community.createEvent} onToggleEvent={community.toggleEvent} onDeleteEvent={community.deleteEvent}/>}

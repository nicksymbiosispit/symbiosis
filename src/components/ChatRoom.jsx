...
function Message({ message, currentUserId, canDeleteAny=false, onDelete, onViewProfile, onReport, onCopyLink, onJumpMessage, onJumpRoom, onJumpBulletin, currentUsername, mentionUsers, availableRooms=[], chatFramesEnabled, linked, copied }) {
  ...
  const parts=message.body.split(/(https?:\/\/[^\s]+|@[A-Za-z0-9_!@]+|#[A-Za-z0-9_-]+)/g);
  const rawSiteReference=siteReference(message.body.trim());
  const directSiteReference=rawSiteReference?.type==='room'&&!availableRooms.some(item=>item.slug===rawSiteReference.room)?null:rawSiteReference;
  return <article id={`message-${message.id}`} className={`message ${linked?'linked-message':''}`}><Avatar profile={message.profile} showFrame={chatFramesEnabled}/><div><div className="message-head">{message.profile?.id ? <button className="message-user user-link" onClick={() => onViewProfile(message.profile)}>{username}</button> : <span className="message-user">{username}</span>}{(message.profile?.tags||[]).map(tag=><span className={`account-tag tag-${tag.toLowerCase().replace(/[^a-z]/g,'')}`} key={tag}>{tag}</span>)}<span className="message-time">{time}</span><button className="message-link" title="Copy message link" onClick={()=>onCopyLink(message.id)}>{copied?'copied':'link'}</button>{(message.user_id===currentUserId||canDeleteAny)&&<button className="message-link danger" onClick={()=>onDelete(message.id)}>delete</button>}<button className="report-link" title="Report message" onClick={()=>onReport(message)}>⚑ report</button></div><div className="message-body">{directSiteReference?.type==='message'?<button type="button" className="message-reference" onClick={()=>onJumpMessage(directSiteReference.room,directSiteReference.messageId)}># {directSiteReference.room}</button>:directSiteReference?.type==='room'?<button type="button" className="message-reference" onClick={()=>onJumpRoom(directSiteReference.room)}># {directSiteReference.room}</button>:directSiteReference?.type==='bulletin'?<button type="button" className="message-reference bulletin-reference" onClick={()=>onJumpBulletin(directSiteReference.bulletinId)}>▣ bulletin #{directSiteReference.bulletinId}</button>:IMAGE_URL_PATTERN.test(message.body)?<><EmbeddedMedia url={message.body} username={username}/>{message.caption&&<div className="media-caption">{message.caption}</div>}</>:parts.map((part,index)=>{const reference=part.startsWith('http')?siteReference(part):null;if(reference?.type==='message')return <button type="button" className="message-reference" key={index} onClick={()=>onJumpMessage(reference.room,reference.messageId)}># {reference.room}</button>;if(reference?.type==='room')return <button type="button" className="message-reference" key={index} onClick={()=>onJumpRoom(reference.room)}># {reference.room}</button>;if(reference?.type==='bulletin')return <button type="button" className="message-reference bulletin-reference" key={index} onClick={()=>onJumpBulletin(reference.bulletinId)}>▣ bulletin #{reference.bulletinId}</button>;const channel=part.startsWith('#')?availableRooms.find(item=>item.slug.toLowerCase()===part.slice(1).toLowerCase()):null;if(channel)return <button type="button" className="message-reference" key={index} onClick={()=>onJumpRoom(channel.slug)}>#{channel.slug}</button>;const person=part.startsWith('@')?mentionUsers.find(user=>user.username.toLowerCase()===part.slice(1).toLowerCase()):null;return person?<button type="button" className={person.username.toLowerCase()===currentUsername?.toLowerCase()?'mention mine':'mention'} key={index} onClick={()=>onViewProfile(person)}>{part}</button>:part})}</div></div></article>;
}

export default function ChatRoom({ room='lobby', roomBanner='', pinnedNotice='', userId, currentUsername, mentionUsers=[], availableRooms=[], chatFramesEnabled=true, targetMessageId=null, messages, status, onSend, onDelete, canDeleteAny=false, onViewProfile, onReport, onJumpMessage, onJumpRoom, onJumpBulletin, onSlowMode, onLoadOlder, hasOlder=false, loadingOlder=false, pingsEnabled, onEnablePings, slowModeSeconds=0, mediaMode='allowed', lastSentAt, serverRemaining=0, isModerator=false }) {
  const draftKey=`symbiosis-draft:room:${userId}:${room}`;
  ...
  const mediaOnly=mediaMode==='images_only';
  ...
  const channelMatch=body.match(/(^|\s)#([A-Za-z0-9_-]*)$/);
  const channelOptions=channelMatch?availableRooms.filter(item=>item.slug.toLowerCase().startsWith(channelMatch[2].toLowerCase())).slice(0,6):[];
  ...
  async function submit(event) {
    ...
    if (await onSend(body,caption)) {setBody('');setCaption('');}
  }
  async function chooseMedia(event){const file=event.target.files?.[0];if(!file)return;try{setMediaStatus('uploading…');const url=await uploadRoomMedia(userId,file);if(await onSend(url,caption)){setCaption('');setMediaStatus('posted');window.setTimeout(()=>setMediaStatus(''),900)}}catch(error){setMediaStatus(error.message)}finally{event.target.value=''}}

  return <section className="chat-column"><div className="panel chat-panel">{roomBanner&&<img className="room-banner" src={roomBanner} alt={`#${room} banner`}/>}<div className="panel-title chat-title"><span><span className="room-dot"/> #{room}</span><span className="tiny-meta">{messages.length} loaded</span></div>
    {pinnedNotice&&<div className="pinned-room-notice"><b>📌 ROOM NOTICE</b><span>{pinnedNotice}</span></div>}{slowModeSeconds>0&&<div className="slow-banner">Slow mode: {slowModeSeconds}s {isModerator&&'· staff bypass'}</div>}<div className="chat-scroll" aria-live="polite">{hasOlder&&<button className="load-older" disabled={loadingOlder} onClick={onLoadOlder}>{loadingOlder?'Loading…':'↑ Load older messages'}</button>}{!messages.length && <div className="empty-chat"><strong>No messages in #{room}.</strong> Start the conversation.</div>}{messages.map((message)=><Message key={message.id} message={message} currentUserId={userId} canDeleteAny={canDeleteAny} currentUsername={currentUsername} mentionUsers={mentionUsers} availableRooms={availableRooms} chatFramesEnabled={chatFramesEnabled} linked={String(message.id)===String(targetMessageId)} copied={copiedMessageId===message.id} onDelete={onDelete} onCopyLink={copyMessageLink} onJumpMessage={onJumpMessage} onJumpRoom={onJumpRoom} onJumpBulletin={onJumpBulletin} onViewProfile={onViewProfile} onReport={onReport}/>)}<div ref={endRef}/></div>

...
function Message({ message, currentUserId, canDeleteAny=false, onDelete, onViewProfile, onReport, onCopyLink, onJumpMessage, currentUsername, mentionUsers, chatFramesEnabled, linked, copied }) {
...
export default function ChatRoom({ room='lobby', userId, currentUsername, mentionUsers=[], chatFramesEnabled=true, targetMessageId=null, messages, status, onSend, onDelete, canDeleteAny=false, onViewProfile, onReport, onJumpMessage, onSlowMode, pingsEnabled, onEnablePings, slowModeSeconds=0, mediaMode='allowed', lastSentAt, serverRemaining=0, isModerator=false }) {
  ...
  return <section className="chat-column"><div className="panel chat-panel"><div className="panel-title chat-title"><span><span className="room-dot"/> #{room}</span><span className="tiny-meta">{messages.length} message{messages.length===1?'':'s'}</span></div>
    {slowModeSeconds>0&&<div className="slow-banner">Slow mode: {slowModeSeconds}s {isModerator&&'· staff bypass'}</div>}<div className="chat-scroll" aria-live="polite">{!messages.length && <div className="empty-chat"><strong>No messages in #{room}.</strong> Start the conversation.</div>}{messages.map((message)=><Message key={message.id} message={message} currentUserId={userId} canDeleteAny={canDeleteAny} currentUsername={currentUsername} mentionUsers={mentionUsers} chatFramesEnabled={chatFramesEnabled} linked={String(message.id)===String(targetMessageId)} copied={copiedMessageId===message.id} onDelete={onDelete} onCopyLink={copyMessageLink} onJumpMessage={onJumpMessage} onViewProfile={onViewProfile} onReport={onReport}/>)}<div ref={endRef}/></div>

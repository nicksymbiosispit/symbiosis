...
import StickerEditor, { ProfileStickers } from './StickerEditor.jsx';
...
export default function ProfilePage({ currentUser, profile, viewedProfile, isOnline, friendsApi, safety, onUpdated, onMessage, onReport }) {
  ...
  if (!own) {
    ...
    return <section className="profile-space" style={{ '--space-accent': shown.accent_color || '#39ff14', '--space-bg': shown.background_color || '#080b10' }}><ProfileStickers stickers={shown.stickers}/>
      <div className="profile-banner"><Avatar profile={shown} size="large"/><div><h1>{shown.username}'s space {shown.role==='moderator'&&<span className="mod-badge">MOD</span>}</h1><p>{shown.mood || 'just hanging out online'}</p><span className={isOnline(shown.id)?'presence-online':'presence-offline'}>● {isOnline(shown.id)?'Online now':'Offline'}</span></div></div>
  ...
  async function chooseAvatar(event) {
    ...
    try { setStatus('Uploading image…'); const avatar_url = await uploadAvatar(currentUser.id, file); setForm(current=>({...current,avatar_url})); setStatus('Uploaded — press Save my space.'); }
    catch(error){ setStatus(error.message); }
  }
  async function chooseMusic(event) {
    ...
    try {
      ...
      setStatus('Song uploaded — press Save my space.');
    } catch(error) { setStatus(error.message); }
  }

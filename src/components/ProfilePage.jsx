...
export default function ProfilePage({ currentUser, profile, viewedProfile, isOnline, friendsApi, safety, onUpdated, onMessage, onReport, onViewProfile }) {
  ...
  const [form, setForm] = useState({
    ...
    player_style: profile.player_style || 'terminal',status_mode:profile.status_mode||'online',away_message:profile.away_message||'',profile_html:profile.profile_html||'',entry_sound:Boolean(profile.entry_sound),cursor_style:profile.cursor_style||'classic_green'
  });
  ...
  if (!own) {
    ...
    return <section className={`profile-space cursor-${shown.cursor_style||'classic_green'}`} style={{ '--space-accent': shown.accent_color || '#39ff14', '--space-bg': shown.background_color || '#080b10' }}>

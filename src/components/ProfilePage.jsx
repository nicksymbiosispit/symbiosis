...
import { downloadAccountExport, loadLoginHistory } from '../services/accountData.js';
...
export default function ProfilePage({ currentUser, profile, viewedProfile, isOnline, friendsApi, safety, onUpdated, onMessage, onReport, onViewProfile }) {
  ...
  const [form, setForm] = useState({
    ...
    player_style: profile.player_style || 'terminal',status_mode:profile.status_mode||'online',away_message:profile.away_message||'',profile_html:profile.profile_html||'',entry_sound:Boolean(profile.entry_sound),profile_layout:profile.profile_layout||'classic',cursor_style:profile.cursor_style||'classic_green'
  });
  const [loginHistory,setLoginHistory]=useState([]);
  useEffect(()=>{if(own)loadLoginHistory(currentUser.id).then(setLoginHistory).catch(()=>setLoginHistory([]))},[own,currentUser.id]);
  ...
  if (!own) {
    ...
    return <section className={`profile-space profile-layout-${shown.profile_layout||'classic'} cursor-${shown.cursor_style||'classic_green'}`} style={{ '--space-accent': shown.accent_color || '#39ff14', '--space-bg': shown.background_color || '#080b10' }}>
      <div className="profile-banner"><Avatar profile={shown} size="large"/><div><h1>{shown.username}'s symbiosis</h1><div className="account-tags">{(shown.tags||[]).map(tag=><span className={`account-tag tag-${tag.toLowerCase().replace(/[^a-z]/g,'')}`} key={tag}>{tag}</span>)}</div><p>{shown.mood || 'just hanging out online'}</p><span className={`status-icon status-${shown.status_mode||'online'}`}>● {statusLabel(shown,isOnline(shown.id))}</span></div></div>

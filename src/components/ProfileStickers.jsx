...
export function StickerLayer({stickers=[],editable=false,selectedId,onSelect,onMove}){
  ...
  return <div ref={layerRef} className={`profile-sticker-layer ${editable?'editable':''}`}>{safeStickers(stickers).map(sticker=>{const item=STICKER_CATALOG.find(entry=>entry.id===sticker.type);return <button type="button" key={sticker.id} className={`profile-sticker sticker-${sticker.type} ${selectedId===sticker.id?'selected':''}`} style={{left:`${sticker.x}%`,top:`${sticker.y}%`,transform:`translate(-50%,-50%) rotate(${sticker.rotation||0}deg) scale(${sticker.scale||1})`}} onPointerDown={event=>begin(event,sticker)} onPointerMove={event=>move(event,sticker)} onClick={()=>editable&&onSelect(sticker.id)} tabIndex={editable?0:-1}>{item.art}</button>})}</div>;
}

export default function StickerEditor({stickers=[],onChange,children}){
  const [selectedId,setSelectedId]=useState(null);
  const safe=safeStickers(stickers);
  function add(type){if(safe.length>=30)return;const id=`${Date.now()}-${Math.random().toString(16).slice(2)}`;onChange([...safe,{id,type,x:50,y:50,rotation:0,scale:1}]);setSelectedId(id)}

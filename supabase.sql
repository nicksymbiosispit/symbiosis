...
create or replace function public.my_lobby_cooldown_remaining()
returns integer
language sql
stable
security definer
...
as $$
  with timing as (
    select
      (select max(created_at) from public.messages where user_id = auth.uid()) as last_at,
...
revoke all on function public.my_lobby_cooldown_remaining() from public;
grant execute on function public.my_lobby_cooldown_remaining() to authenticated;
...
begin
  ...
  from public.messages where user_id = new.user_id;
